...
function BorrowPage() {
  ...
  const handleItemPaymentChange = (index, e) => {
    ...
    updatedItems[index].amount = Math.min(newAmount, updatedItems[index].remaining);
  ...
  const handleTotalPaymentChange = (e) => {
    const { value } = e.target;
    const newAmount = parseFloat(value) || 0;
    const totalRemaining = paymentData.items.reduce((sum, item) => sum + item.remaining, 0);
  ...
  const handlePartialPayment = async (e) => {
    ...
    try {
      await axios.patch(`/api/borrows/${selectedBorrow._id}/partial-payment`, {
        ...
        items: paymentData.items.filter(item => item.amount > 0).map(item => ({
          itemId: item.itemId,
          amount: item.amount
        })),
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-8">Система учета займов</h1>
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        <table className="min-w-full">
          ...
          <tbody>
            {borrows.map(borrow => (
              <tr key={borrow._id} className="border-t border-gray-200 hover:bg-gray-50">
                <td className="py-3 px-4">{borrow.lenderName}</td>
                <td className="py-3 px-4">{borrow.items.length}</td>
                <td className="py-3 px-4">{new Date(borrow.borrowDate).toLocaleDateString()}</td>
                ...
                <td className="py-3 px-4">
                  <span className={`px-2 py-1 rounded-full text-xs ${borrow.returned ? 'bg-green-200 text-green-800' : 'bg-yellow-200 text-yellow-800'}`}>
                    {borrow.returned ? 'Возвращено' : calculateRemainingAmount(borrow) > 0 ? 'Частично оплачено' : 'В долгу'}
                  </span>
                </td>
                <td className="py-3 px-4">
                  <button
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <form onSubmit={handleSubmit}>
                <div className="mb-4">
                  <label className="block text-gray-700 text-sm font-bold mb-2">Имя кредитора:</label>
                  <input
                ...
                <div className="mb-6">
                  <label className="block text-gray-700 text-sm font-bold mb-2">Дата возврата:</label>
                  <input
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    type="date"
                    name="returnDate"
                ...
                {formData.items.map((item, index) => (
                  <div key={index} className="grid grid-cols-12 gap-3 mb-4 items-end">
                    <div className="col-span-7">
                      <label className="block text-gray-700 text-sm font-bold mb-2">Название товара</label>
                      <input
                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                        type="text"
                        name="itemName"
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <form onSubmit={handlePartialPayment}>
                <div className="mb-6">
                  <label className="block text-gray-700 text-sm font-bold mb-2">Общая сумма к оплате:</label>
                  <input
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    type="number"
                    min="0"
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    Доступно для оплаты: {calculateRemainingAmount(selectedBorrow).toLocaleString('ru-RU', { 
                      style: 'currency', 
                      currency: 'USD',
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2
                ...
                <div className="mb-4">
                  <label className="block text-gray-700 text-sm font-bold mb-2">Способ оплаты:</label>
                  <select
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    name="paymentMethod"
                ...
                <div className="mb-6">
                  <label className="block text-gray-700 text-sm font-bold mb-2">Дата оплаты:</label>
                  <input
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    type="date"
                    name="paymentDate"
                ...
                <h3 className="text-lg font-semibold mb-3">Распределение оплаты по товарам:</h3>
                {paymentData.items.map((item, index) => (
                  item.remaining > 0 && (
                    <div key={index} className="grid grid-cols-12 gap-3 mb-4 items-end">
                      <div className="col-span-5">
                        <label className="block text-gray-700 text-sm font-bold mb-2">Товар</label>
                        <p className="text-gray-900">{item.itemName}</p>
                      </div>
                      <div className="col-span-3">
                        <label className="block text-gray-700 text-sm font-bold mb-2">Остаток</label>
                      ...
                      <div className="col-span-3">
                        <label className="block text-gray-700 text-sm font-bold mb-2">Сумма оплаты</label>
                        <input
                          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                          type="number"
                          min="0"
            ...
            <div className="p-6">
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{new Date(selectedBorrow.borrowDate).toLocaleDateString()}</p>
                  </div>
                  <div>
                    <p className="text-gray-600 text-sm">Дата возврата:</p>
                    <p className="font-medium">
                      {selectedBorrow.returnDate ? new Date(selectedBorrow.returnDate).toLocaleDateString() : 'Не указана'}
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-600 text-sm">Статус:</p>
                    <p className="font-medium">
                      <span className={`px-2 py-1 rounded-full text-xs ${selectedBorrow.returned ? 'bg-green-200 text-green-800' : 'bg-yellow-200 text-yellow-800'}`}>
                        {selectedBorrow.returned ? 'Возвращено' : calculateRemainingAmount(selectedBorrow) > 0 ? 'Частично оплачено' : 'В долгу'}
                      </span>
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg mb-6">
                <div className="flex justify-between font-bold text-lg mb-2">
                  <span>Общая сумма:</span>
                  <span>
                    {calculateBorrowTotal(selectedBorrow).toLocaleString('ru-RU', { 
                      style: 'currency', 
                      currency: 'USD',
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2
                ...
                <div className="flex justify-between font-bold text-lg">
                  <span>Остаток:</span>
                  <span>
                    {calculateRemainingAmount(selectedBorrow).toLocaleString('ru-RU', { 
                      style: 'currency', 
                      currency: 'USD',
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2
              ...
              <div className="flex justify-end space-x-3">
                {!selectedBorrow.returned && calculateRemainingAmount(selectedBorrow) > 0 && (
                  <button
                    onClick={() => {
                      setShowDetailModal(false);
